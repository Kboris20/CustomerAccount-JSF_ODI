package ch.hearc.ig.odi.serie6_b.services;

import ch.hearc.ig.odi.serie6_b.business.Account;
import ch.hearc.ig.odi.serie6_b.business.Bank;
...
public class Services implements Serializable {
    ...
    public Services() throws CustomerAlreadyExistException, AccountAlreadyExistException {
        bank = new Bank(1, "Raiffeisen Neuchâtel");

        Customer cust = bank.addCustomer(1, "Jean", "Dupont");

        bank.addAccount("1", "Salaire", 0.001, cust);
        bank.addAccount("2", "Impots", 2000, 0.002, cust);

        cust = bank.addCustomer(2, "Marie", "Dupont");
        bank.addAccount("3", "Epargne", 10000, 0.002, cust);
    ...
    public List<Account> getAccountsByCust(Integer cust_id) {
        return bank.getAccountsByCust(cust_id);
    }

    public Account getAccount(Integer id_account) {
        return bank.getAccountById(id_account);
    }
}
